...
class CombinedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.resnet_features[idx], self.lbp_features[idx], self.labels[idx]
        ...
        for phase in ['train', 'val']:
            ...
            for batch in dataloaders[phase]:
                if model_type == 'lbp' or model_type == 'resnet50':
                    ...
                    # Combine inputs for consistency
                    inputs = (resnet_inputs, lbp_inputs)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    if model_type == 'lbp' or model_type == 'resnet50':
                        ...
                        resnet_outputs = model[0](resnet_inputs)  # Forward pass through ResNet
...
if __name__ == "__main__":
    ...
    print(f"[INFO] model_type12345678: {model_type}")
    ...
    if model_type == 'lbp':
        ...
        # Reshape and permute only the image part for ResNet50 + LBP combined features
        resnet_features_train = torch.tensor(train_data[:, :-26].reshape(-1, 3, 224, 224), dtype=torch.float32)
        lbp_features_train = torch.tensor(train_data[:, -26:], dtype=torch.float32)
        combined_train_data = [resnet_features_train, lbp_features_train]

        resnet_features_test = torch.tensor(test_data[:, :-26].reshape(-1, 3, 224, 224), dtype=torch.float32)
        lbp_features_test = torch.tensor(test_data[:, -26:], dtype=torch.float32)
        combined_test_data = [resnet_features_test, lbp_features_test]
    else:
    ...
    if model_type == 'resnet50_lbp':
        print("[INFO] Data shape after processing:", resnet_features_train.shape, lbp_features_train.shape)
    ...
    if model_type == 'lbp':
        ...
        train_dataset = CombinedDataset(combined_train_data[0], combined_train_data[1], torch.tensor(y_train, dtype=torch.long))
        test_dataset = CombinedDataset(combined_test_data[0], combined_test_data[1], torch.tensor(y_test, dtype=torch.long))
    else:
        train_dataset = TensorDataset(train_data, torch.tensor(y_train, dtype=torch.long))
        test_dataset = TensorDataset(test_data, torch.tensor(y_test, dtype=torch.long))
    ...
    if model_type == 'lbp':
        ...
        lbp_features_test = torch.tensor(test_data[:, -26:], dtype=torch.float32)
        combined_test_data = [resnet_features_test, lbp_features_test]
        test_dataset = CombinedDataset(combined_test_data[0], combined_test_data[1], torch.tensor(test_labels, dtype=torch.long))
    else:
