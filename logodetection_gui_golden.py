...
def predict_logo(classifier, resnet, lda, scaler, device, label_dict, image_path):
    ...
    combined_features_lda = lda.transform([combined_features])
    combined_features_lda = scaler.transform(combined_features_lda)

    inputs = torch.tensor(combined_features_lda, dtype=torch.float32).to(device)
...
def predict_logos_in_folder(classifier, resnet, lda, scaler, device, label_dict, folder_path):
    ...
    for subdir, _, files in os.walk(folder_path):
        ...
        for filename in files:
            if filename.endswith(('.jpg', '.jpeg', '.png')):
                ...
                predicted_label = predict_logo(classifier, resnet, lda, scaler, device, label_dict, image_path)
...
if __name__ == "__main__":
    ...
    combined_features = np.hstack((resnet_feature, lbp_feature))
    combined_features_lda = lda.transform([combined_features])
    in_features = combined_features_lda.shape[1]
    ...
    predict_logos_in_folder(classifier, resnet, lda, scaler, device, label_dict, folder_path)
